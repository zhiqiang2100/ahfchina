...
*/
Class Controller_Aj_Artfavorite {
    ...
    private function getParam() {
		 if(empty($_SERVER['HTTP_REFERER']) || strpos($_SERVER['HTTP_REFERER'],'sina.cn')===false)
        {
            $ret = array(
                'code'  => 0,
                'msg'   => '请求非法',
        ...
        $this->_param['fid']   = (int)Be_Data::param('fid', 0); 
        //收藏动作 string 'add'：添加 'isFav'：是否收藏
        $this->_param['op']     = Be_Data::param('op', 'isFav');
        //文章或者图片的id
        $this->_param['docid']  = htmlspecialchars(Be_Data::param('docid', ''));
       $this->_param['uid']    = Model_Common::getUserId();//( (int)Be_Data::param('uid', 0)  > 0 ) ? (int)Be_Data::param('uid', 0) : Model_Common::getUserId();
	...
	case 'del':            
           if(!empty($this->_param['fid'])) $result = Model_Favorite::delFavorite($this->_param['uid'], $this->_param['fid'], $this->_param['type']);
		$ret = array(
				'status'    => 1,
				'message'   => 'success',
				'data'  => $result,
    ...
    private function checkOp() {
        return in_array($this->_param['op'], array('add', 'del','isFav'));
    }
